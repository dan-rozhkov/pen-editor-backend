...
bringing their own skills, server-side product analytics, prototype photography
that is generated rather than borrowed, per-IP rate limiting on the paid routes,
and the schema half of point edits to a screen's HTML.

### Added
- **`read_embed_html` and `edit_embed_html` schemas.** Editing part of an existing embed screen meant rewriting its whole `htmlContent` through `batch_design` — thousands of tokens each way, a real risk of a truncated generation, and silent drift in parts nobody asked to change. Both tools are client-executed (handlers in pen-editor v0.77.0): `read_embed_html` serves outline/grep/full views over a screen's HTML, `edit_embed_html` applies exact-anchor `oldString`→`newString` edits atomically. The system prompt and the `prototype`/`slides` skills now route a partial edit to them; `batch_design` `U(id, {htmlContent})` is reserved for replacing a screen wholesale. Spec: `docs/specs/2026-08-19-incremental-embed-html-editing-design.md`.
- **The L2 scenario layer, between offline analysis and the background review.** The review decided "save or not" from a single conversation and therefore almost always declined — one session cannot show that a pattern recurs — while the analysis pipeline saw every session but reported only to humans. `npm run analyze` now mines L1 insight atoms into `agent_scenarios`: patterns confirmed by several **distinct** sessions, deduplicated by title embedding (exact normalized title first, cosine distance second, so a deployment without `EMBEDDINGS_API_KEY` still converges). `maybeRunReview` gains a third due-source — a confirmed scenario carries a review on its own and rides in as evidence. Scenarios travel in the review's user message, never the system prompt, which is reused verbatim to keep the provider prefix cache warm; a scenario is offered at most twice, enforced in the fetch query rather than only in settle, since the failure path never reaches settle; scenario-triggered reviews are capped at one per user per hour, or a backlog of N would fire N back-to-back model runs; and only a user's own scenarios reach the prompt, as the global pool's text is an instruction with no confirmed owner. The report now splits review saved-rate by whether the run had scenario evidence — the number this layer is judged by. Spec: `docs/specs/2026-08-20-selfimprove-l2-scenarios-design.md`.
- **Users can bring their own skills.** A third skill source beside the curated `src/skills/*.md` and the agent's own `agent_skills`: `user_skills` (migration 012), keyed by the same anonymous `userId` as `agent_memory`, holding skills a user writes, uploads as Markdown, or has the agent draft. Resolution is curated > user > learned. Unlike learned skills, user skills **are** slash-invocable — `/my-skill` is the point — reusing the existing synthetic lookup pair so prompt caching is untouched; a user with no custom skills renders a byte-identical prompt. Both the catalog read and the slash lookup are raced against a 2s timeout, since the slash path fires on any message starting with `/` and a stalled Postgres connection would otherwise hang `/api/chat` itself. CRUD lives at `/api/user-skills`, plus a generate endpoint that drafts a skill from a workflow description and persists nothing.
- **Server-side product analytics (PostHog)**, a complete no-op unless `POSTHOG_API_KEY` is set, so dev, tests and CI never touch the network. `agent_turn_completed` carries `turn_complete`, because the client auto-resends after every client-executed tool call — without it one user turn looks like N turns with partial durations. No PII: properties are enums, ids, counts and durations, never message text and never a raw error message; a test asserts it. Separate from the `raw_traces`/`session_insights` pipeline, which analyses content rather than funnels.
- **Publish canvas screens to the gallery from the editor** (`POST /api/showcase/publish`) — the first HTTP-reachable caller of `publishScreens`. No headless browser is involved: the screens already exist as pixels on the client's canvas, so the frontend rasterizes each one and POSTs HTML + PNG, and the route validates type/size/dimensions (PNG sniffed rather than trusted) before injecting the client's PNG into the same normalize/derivatives/S3/row pipeline `showcase:generate` uses.
- **Object storage providers whose read host differs from their write host** (`S3_PUBLIC_BASE_URL`, `S3_OBJECT_ACL`, `S3_LEGACY_PUBLIC_BASE_URLS`) — the move from timeweb to Cloudflare R2, whose S3 endpoint only answers signed calls and which rejects `ACL: public-read` outright. The legacy list keeps the back catalogue's photos alive, since already-published HTML still points at the old host.

### Changed
- **The self-improvement loop now fires.** Over its whole production lifetime it had produced four audit rows. Four causes, fixed together: every review run now writes an audit row (so "never fires" and "fires and declines" stopped being indistinguishable); the memory threshold counted completed turns, of which one design message spans 8–12 round-trips, so it drops 10 → 4 and both thresholds move to env-overridable config; `SELF_SKILLS_GUIDANCE` gives the foreground actual skill-writing pressure, asking for patches only; and the review prompt, which asked solely about the user, gained a third focus on the agent's own operating knowledge — the reason nothing was ever written to `target='memory'`.
- **Prototype photography is generated, not borrowed.** The rule lived in the showcase runner's user message, so users typing `/prototype` in chat kept getting picsum stock frames. It moves into the shared `prototype`/`slides` skills, which both chat and the headless runner read: list the shots, issue the calls in parallel, paste the URLs — ~8 per design, with micro imagery staying on picsum and an explicit fallback order for turns where the tool is absent, errors, or reports a spent budget. `/api/generate-image` goes 10 → 20 req/min/IP, since one design now authors its own photography through it.
- **The prototype skill's mobile preset is 390×844**, matching `SHOWCASE_VIEWPORTS.mobile` and `publish_to_showcase`'s size check. It had told the agent to build at 375×812, so every mobile run came out the wrong size and the last dry run flagged all five screens as overflowing. A test pins the preset against the viewport constant so the two cannot drift silently again.
- **A saved process preference now outranks the default flow.** A memory like "skip the upfront brief, show a first draft" reached the prompt fine but was ignored: `CORE_PROMPT`, `ask_user`'s description and both skills state an unconditional "ask first" rule, and the model followed the stronger instruction. An explicit priority note ahead of the memory snapshot, plus a matching exception in all three places, makes a standing preference legible as an override.
- Default `POSTHOG_HOST` is the EU cloud. A region mismatch fails in the worst way — the wrong region answers 200 to an unknown key and drops the event, which looks exactly like a correctly wired setup with no traffic.

### Fixed
- **`trustProxy: true` let a client forge `request.ip`** and mint a fresh identity per request, bypassing the per-IP limiter entirely. Render is a single proxy hop, so `trustProxy: 1` is the correct, unforgeable setting.
- **The paid, unauthenticated routes had no throttling.** `@fastify/rate-limit` (opt-in per route, global default off) now covers `/api/generate-image`, `/api/prototype-link`, `/api/showcase/publish` and `/api/user-skills/generate` at 10/min/IP.
- **A self-improvement review that timed out or threw wrote no audit row**, so it looked exactly like one that never fired — the confusion the row exists to end. All four outcomes are recorded now; a throw before the review starts still writes nothing, deliberately.
- A flaky showcase test that slept 20ms hoping to win a race now awaits a deferred resolved by the stub itself.
...
### Added
- **`draw_vector`: the agent draws a native vector contour progressively.** A new tool whose input is a newline-separated command script (`M`/`L`/`C`/`CLOSE`/`FILL`/`STROKE`/`END`), declared so the browser can preview each complete line while the tool input is still streaming and commit a single `PathNode` after full validation. Only offered on `native` turns — the embed-only policy (prototype/slides) deletes it in `prepareChatTurn`, so an embed-only run can never be handed a tool that needs a real scene graph.

...
function buildPrompt(screens: PrototypeScreenInput[]): string {
  ...
  return [
    ...
    "  - a user avatar / profile photo / account icon in a header (an `avatar`/`profile`/`account` element, often just an image with no text) → the profile, account, or settings screen if one exists.",
    "  - a `back` control or 'Back' / 'Cancel' → the previous/parent screen the user came from.",
    "Light/dark theme switching: if the screens come in light/dark PAIRS — the same screen present twice, its names differing only by a 'Dark'/'Light'/'Night'/'Day' marker (e.g. 'Settings' and 'Settings Dark') — and a screen has a theme / dark-mode / appearance control (a toggle/switch, or an element labeled 'Dark mode'/'Theme'/'Appearance', often with a moon/sun icon), link that control to the SAME screen's opposite-theme variant (the 'Dark mode' toggle on 'Settings' → 'Settings Dark'; on 'Settings Dark' → 'Settings'). Only do this when the opposite-theme variant actually exists among the screens; otherwise treat the toggle as non-navigational.",
    "Only skip elements that are truly non-navigational (form toggles/checkboxes, 'Delete', and similar in-place controls with no destination).",
...
export async function generatePrototypeLinks(
  ...
  const { object } = await generateObject({
    ...
    // or a user in the composer — picked for chat, and silently degrade
    // into NoObjectGeneratedError territory.

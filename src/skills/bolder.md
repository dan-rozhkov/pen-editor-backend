...
When the target holds its own without pulling the page apart, hand off to the `/polish` skill for the final pass.

## Quality floor

**Verify**
- Contrast: any new emphasis inside the target still meets ≥4.5:1 body / ≥3:1 large-text contrast; secondary text stays tinted from the surface hue, never generic gray.
- Depth: any shadow added to raise the target needs an offset plus soft blur, never a zero-offset colored halo.
- Spacing: the amplified section keeps tight-group/generous-separation rhythm — more space above its heading than below.
- Type: measure stays 65–75ch where prose remains; tracking floor -0.04em.
- Motion: at most one authored moment of emphasis for the target, not decoration copied onto its neighbors.
- States: hover, disabled, loading, error, and empty states for anything interactive inside the target still work and still look considered.
- Copy: amplification is in hierarchy and structure, not in inflating the words; existing claims stay honest.

**Refuse**
- Adding gradients, glassmorphism, neon accents, or a color-stripe border to "look bolder" — those are the reflex answer this skill exists to reject.

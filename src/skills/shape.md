...
**Output**: A design brief that can be handed off to the `/craft` skill, or used directly to guide freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
This is a required interaction, not optional guidance. Ask these questions in conversation, adapting based on answers. Don't dump them all at once; have a natural dialogue. STOP and {{ask_instruction}} to clarify.

### Interview cadence

Discovery includes at least one user-answer round unless the project's product context, the project's design system, or an already-confirmed brief directly answers the needed inputs. With a sparse prompt, do **not** synthesize a complete brief for confirmation on the first response.

- {{ask_instruction}} directly and stop for the answers.
- Ask **2-3 questions per round**, then wait for answers.
- Treat the project's product context and design system as anchors; they reduce repeated questions but do **not** replace shape for craft. Shape is task-specific.
...
**Assert-then-confirm, not menu-with-escape.** When the project's product context and the user's prompt make one option obvious, name it and ask the user to confirm or override. Don't enumerate "Restrained / Committed / Or something else?" as a real choice; "This reads as Restrained, confirm?" beats a four-option menu when the answer is already clear.
...
Force a visual decision on three fronts. Skip anything the project's product context or design system already answers; ask only what's missing.
...
Scope answers are task-scoped. Don't write them to the project's product context or design system; carry them through the design brief only.
...
**3. Design Direction**
Color strategy (Restrained / Committed / Full palette / Drenched) + the theme scene sentence + 2–3 named anchor references. Reference the project's product context and design system where they already answer, and note any per-surface overrides.
...
Based on the brief, list which reference skills would be most valuable during implementation (e.g., the `/layout` skill for complex layouts, the `/animate` skill for animated features, the `/typeset` skill for typography-driven pages, the `/colorize` skill for color-led brands). For form-heavy features, note the interaction and form-design guidance that should be consulted.
...
STOP and {{ask_instruction}} to clarify. Ask for explicit confirmation of the brief before finishing.
...
Once confirmed, the brief is complete. The user can now use it to guide any implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use the `/craft` skill instead, which runs this shaping process internally.)

...
Brand: run the font selection procedure in the `/brand` skill. Fluid `clamp()` scale, ≥1.25 ratio between steps.
...
When the type carries the hierarchy on its own, hand off to `/polish` for the final pass.

---

## Reference Material

The sections below provide the deep typography reference for this flow.
...
The tactical selection procedure and the reflex-reject list live in the `/brand` skill under **Font selection procedure** and **Reflex-reject list** (loaded for brand-register tasks). The rest of this section covers the adjacent knowledge: anti-reflex corrections, system font use, and pairing rules.

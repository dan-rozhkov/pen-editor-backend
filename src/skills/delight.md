...
Inspect the target, the project's captured visual system (see `/document`), product voice, repeated-use frequency, and emotional context. Look for:
...
Do not manufacture a celebration for an ordinary click. If the brand's emotional range or the stakes cannot be inferred, {{ask_instruction}}.
...
For authored motion, load `/animate`. Respect screen readers, keyboard use, touch, localization, and cultural context. Nonessential loops stop when hidden. Make celebration intensity proportional to frequency and consequence.

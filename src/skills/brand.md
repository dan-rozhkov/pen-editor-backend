---
name: brand
description: Brand-register design guidance for surfaces where design IS the product (marketing, landing pages, campaigns, portfolios).
user-invokable: true
---
...
Distinctive + refined is the goal. The specific shape depends on the brand, not on the brand's category. A category ("restaurant", "dev tool", "magazine", "fintech") is not a recipe; treating it as one is the first-order reflex the frontend-design skill warns against.
...
- **For greenfield work without local assets, use stock imagery.** `picsum.photos` is the default and DOES resolve reliably — use the seed shape `https://picsum.photos/seed/{unique_string}/{w}/{h}` (a distinct seed per image keeps them stable and varied). A real product/place photo URL supplied by the user is always preferred when you have one. Never substitute colored `<div>` placeholders where a photo belongs. (If you instead reach for Unsplash `https://images.unsplash.com/photo-{id}?...`, verify each URL resolves via an image-search MCP, web-fetch, or browser — guessed Unsplash IDs 404 and ship as broken images.)

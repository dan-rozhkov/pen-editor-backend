...
Build the assigned direction, not a safer interpretation of it. The form supplies structure, reading order, component conventions, and native motion; the product supplies every fact. Commit every atom: nav, buttons, inputs, and links are rebuilt in the form's own vocabulary — a stock, undecorated component inside a committed world is a lapse (reuse the established markup/CSS patterns already used elsewhere in the design, but style the surface around them, not the reverse).

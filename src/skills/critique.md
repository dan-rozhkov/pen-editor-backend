...
Resolve one stable target, run two independent assessments, synthesize a design critique, and {{ask_instruction}} what to improve next. The chat response is the primary deliverable.
...
- Do not claim visual evidence exists unless you actually inspected the rendered design.

### Setup

**Resolve the target** to a concrete frame, component, or region of the scene.
- "the homepage" -> the frame that renders it
- "the settings modal" -> the primary component embed
- "this screen" -> the currently selected frame or node
...
### Assessment B: Anti-Pattern Scan + Visual Evidence
...
Visual evidence:
- Visually inspect the rendered design (e.g., capture a screenshot of the relevant frames) to catch issues that are not visible from the structure alone — spacing, contrast, alignment, and overall composition.
- For multi-view targets, inspect 3-5 representative screens.

Return: the list of anti-pattern findings with counts and locations, visual observations, and any false positives.

After Assessment B returns usable findings, reuse them. Do not redo the anti-pattern scan in the parent unless Assessment B failed, was truncated, or omitted counts, rule names, or locations.

### Generate Combined Critique Report

Synthesize both assessments into a single report. Do NOT simply concatenate. Weave the findings together, noting where the design review and the anti-pattern scan agree, where the scan caught issues the review missed, and where scan findings are false positives.
...
Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If the project's product context includes audience/brand information, also generate 1-2 project-specific personas from it.
...
**After presenting findings**, use targeted questions based on what was actually found. STOP and {{ask_instruction}} to clarify. These answers will shape the action plan.
...
If the project's product context describes the target audience and brand, derive 1–2 additional personas from that information:
...
**Profile**: [2-3 key characteristics derived from the product context]

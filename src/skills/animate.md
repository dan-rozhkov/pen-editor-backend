...
Respect the project's design system (design tokens/variables, existing components, and established visual conventions) before inventing new motion. Reuse any established duration/easing tokens and existing transition patterns where they exist; only add new motion primitives when the current system can't express the effect.
...
When the motion clarifies state instead of decorating it, hand off to `/polish` for the final pass.

...
Respect the project's design system (design tokens/variables, existing components, and established visual conventions) before inventing new spacing. Reuse the established spacing scale and layout patterns where they exist; only extend the scale when the current tokens genuinely can't express the rhythm you need.
...
When the rhythm and hierarchy land, hand off to `/polish` for the final pass.

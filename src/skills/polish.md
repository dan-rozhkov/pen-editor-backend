...
Automated QA output is defect evidence only. A clean automated result is never proof that the design is strong; gather visual evidence and inspect the real interaction path against the anti-patterns in the frontend-design skill.
...
1. **Find the design system**: Review the project's design system — design tokens/variables (the `get_variables` tool), existing scene nodes, and reusable component embeds. Study the core patterns: design principles, target audience, color tokens, spacing scale, typography styles, component API, motion conventions.
2. **Note the conventions**: How are shared components reused? What spacing scale is used? Which colors come from tokens vs hard-coded values? What motion and interaction patterns are established? What flow shapes are used for comparable actions (modal vs full-page, inline vs route, save-on-blur vs explicit submit)?
...
If a design system exists, polish **must** align the feature with it. If none exists, polish against the conventions visible in the existing scene. **If anything about the system is ambiguous, {{ask_instruction}}. Never guess at design system principles.**
...
4. **Pull in any prior critique** (optional signal): If `/critique` has been run on the same target, its priority issues are a useful prior for what to address first. Fold any prior P0/P1 items into your polish list. The critique is one input among many — do your own pass either way.
...
- **Treat automation carefully**: Run any available QA checks when relevant, fix their defects, but never cite a clean result as proof that the work is polished.

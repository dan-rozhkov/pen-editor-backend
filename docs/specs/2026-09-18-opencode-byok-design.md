...
**Zen** подключается как полноценный маршрут (вторая база + префикс
`opencode/`) и получает **свои записи** в `DEFAULT_MODELS`. Без них он был
бы мёртвым кодом: серверного ключа нет, `CHAT_MODEL` не может быть
opencode-ссылкой, значит единственный путь к такой модели — пик из списка.
На `/chat/completions` у Zen 20 моделей (таблица эндпоинтов в
`opencode.ai/docs/zen/`, снята 2026-09-18); в пикер выносим три:
`deepseek-v4-flash`, `glm-5.3-flash`, `kimi-k2.7-code`. Ключ из консоли Zen
предполагается общим на оба маршрута — это подтверждает живой смоук; если
окажется, что нет, в диалоге появится второе поле.
...
Восемь записей: пять с меткой «· Go», три с меткой «· Zen». `supportsVision` = **false** всем, кроме

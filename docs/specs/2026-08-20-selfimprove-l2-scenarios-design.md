...
дефолт 3), `state IN ('open','offered')` и `offer_count < 2` — кап живёт
в самой выборке, а не только в `settleScenarios`: путь падения/таймаута
ревью до `settleScenarios` не доходит, и без фильтра в `WHERE` строка
осталась бы due навсегда.

**Кулдаун:** если этому пользователю уже что-то предлагалось меньше часа
назад, сценарии не выдаются вообще. Иначе бэклог из N намытых сценариев
запускал бы N полных фоновых прогонов модели на N подряд идущих ходах —
ровно то, что счётчики `MEMORY_REVIEW_INTERVAL`/`SKILL_REVIEW_INTERVAL`
существуют, чтобы ограничивать.

**В ревью попадают только сценарии самого пользователя** (`user_id = $1`).
Глобальный пул продолжает намываться и виден в отчёте анализа, но в промпт
не идёт: его title/recipe — инструкция, намытая из чужих неатрибутированных
сессий, а `scrubPii` вычищает идентификаторы, не инструкции. Попав в промпт,
она могла бы быть записана в персональную память ЭТОГО пользователя.
